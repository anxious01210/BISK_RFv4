...
"""Backfill AttendanceEvent.person_id from student via StudentProfile.legacy_student.

M2 of the attendance identity adoption (Phase 1.5 of the Person architecture).

For each AttendanceEvent row whose ``person_id`` is NULL and whose ``student_id``
...
The actual logic lives in ``_0035_helper`` so that tests can import it
(this module's name starts with a digit and is not importable normally).
"""
from django.db import migrations

from . _0035_helper import forwards, backwards  # noqa: F401


class Migration(migrations.Migration):

    dependencies = [
        ("attendance", "0034_attendanceevent_person"),
    ]
