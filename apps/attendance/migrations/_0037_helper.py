...
"""Helper functions for the 0037 AttendanceRecord.person_id backfill migration.

Lives outside the ``0037_attendancerecord_person_backfill.py`` module (whose
...
Strategy:
  ...
  * Iterate ``AttendanceRecord`` rows needing backfill
  ...
  * For each batch, group rows by ``person_id`` and issue one
    ``update(pk__in=rec_ids, person_id=person_id)`` per group. This avoids
    per-row ``save()`` and minimizes write traffic on the largest attendance
    table.
  * Rows whose Student is not yet migrated to identity are skipped —
    ``person`` stays NULL (non-fatal; legacy ``student`` path keeps working).
...
def forwards(apps, schema_editor):
    ...
    pending_by_person = {}  # person_id -> [record_id, ...]
    ...
    for row in qs.iterator(chunk_size=BATCH_SIZE):
        ...
        if total >= BATCH_SIZE:
            _flush(AttendanceRecord, pending_by_person)
...
def _flush(model, pending_by_person):
    """Issue one ``update(pk__in=rec_ids, person_id=...)`` per person_id group."""
    for person_id, rec_ids in pending_by_person.items():
        model.objects.filter(pk__in=rec_ids).update(person_id=person_id)


def backwards(apps, schema_editor):
    # Noop reverse — the person_id column is dropped by the reverse of
    # migration 0036 (RemoveField). We deliberately do NOT null out
    # person_id here so that a rollback-then-re-apply of 0037 is a no-op
    # (idempotent), and a rollback of 0036 cleanly drops the column.
    pass
