...
class FaceEmbeddingAdmin(admin.ModelAdmin):
    ...
    def re_enroll_capture_run(self, request, pk: int):
        ...
        # session = request.POST.get("preview_session") or request.GET.get("preview_session")
        # if session:
        #     # Build base URL from the current request (e.g., http://127.0.0.1:8000 or https://dev.example.com)
        #     scheme = "https" if request.is_secure() else "http"
        #     server_base = f"{scheme}://{request.get_host()}"
        #
        #     # --- HARD STOP the standalone preview so frames don't interleave with capture ---
        #     stop_url = f"{server_base}/attendance/stream/run/stop/{session}/"
        #     try:
        #         try:
        #             import requests  # optional, if available
        ...
        #         pass
        #     else:
        #         # small breather so the runner can wind down
        #         import time as _t;
        #         _t.sleep(0.25)
        # # ✨ Reuse the live-preview session (if the modal provided one)
        # # session = request.POST.get("preview_session") or request.GET.get("preview_session")
        # # if session:
        # #     # Build base URL from the current request (e.g., http://127.0.0.1:8000 or https://dev.example.com)
        # #     scheme = "https" if request.is_secure() else "http"
        # #     server_base = f"{scheme}://{request.get_host()}"
        #
        #     # Optional auth key (matches STREAM_UPLINK_KEY in settings.py)
        #     uplink_key = getattr(settings, "STREAM_UPLINK_KEY", "")
        #
        #     # Append flags for our capture script (these are no-ops if script was not patched)
        #     cmd += ["--preview-session", session, "--server", server_base]
        #     if uplink_key:
        #         cmd += ["--uplink-key", uplink_key]
        #
        #     # Optional: choose a conservative preview post rate/quality (kept *independent* of capture params)
        #     cmd += ["--uplink-maxfps", "6", "--uplink-quality", "85"]
        #     cmd += ["--no-window"]

        # Session used by the live-capture overlay stream (different from the preview)
        prev_session = request.POST.get("preview_session") or request.GET.get("preview_session") or ""
        # Always use a NEW session for capture overlays to avoid interleaving with preview
        cap_session = (prev_session.strip() + "_cap") if prev_session else (f"cap_{uuid4().hex[:6]}_cap")
        ...
        # Best-effort: stop the old preview runner so it stops posting to the old session
        if prev_session:
            stop_url = f"{server_base}/attendance/stream/run/stop/{prev_session}/"
            try:
                try:
                    import requests
                    requests.get(stop_url, timeout=1)
        ...
        # Append preview/uplink flags FOR THE NEW CAPTURE SESSION
