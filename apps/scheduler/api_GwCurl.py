...
def heartbeat(request):
    ...
    try:
        prof_id = int(prof_id) if prof_id is not None else None
    except Exception:
        prof_id = None
    pid = payload.get("pid")
    ...
    if not rp and cam_id:
        # Fallback when profile_id is missing
        rp = (RunningProcess.objects
              .filter(camera_id=cam_id)
              .order_by("-id").first())

    # Back-fill any missing identifiers so we always touch the correct RP
    if rp:
        cam_id = cam_id or rp.camera_id
        prof_id = prof_id or rp.profile_id
        pid = pid or rp.pid

        # Refresh last_heartbeat (and last_heartbeat_at if present)
        now = timezone.now()
        update_fields = []
